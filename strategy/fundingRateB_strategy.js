...
class FundingRateBStrategy extends StrategyBase {
        ...
        request.get({
            ...
            that.cfg["rates"].forEach(rate => {
                ...
                items.forEach(item => {
                    ...
                    subscription_list.push(`BinanceU|${symbol}|perp|trade`, `BinanceU|${symbol}|perp|bestquote`);
                    that.cfg["symbols"].push(symbol);

                    that.cfg["mins"].forEach(min_str => {
                        let entry = `${item.symbol}_${min_str}_${rate_str}`;
    ...
    load_tick_size(idf) {
        ...
        logger.info(`${that.alias}::Loading the tick size from ${url}`);
        request.get({
            url: url, json: true
        }, function (error, res, body) {
            ...
            if (infos.length === 1) {
                ...
                that.slack_publish({
                    "type": "alert",
                    "msg": `${moment().format("HH:mm")}|${that.alias}::${idf} tick size info updated!`
                });
            } else {
                that.slack_publish({
                    "type": "alert",
                    "msg": `${moment().format("HH:mm")}|${that.alias}::${idf} tick size info not right::${infos}`
                });
            }
        });
    }
